...
size_t	ft_print_str(char *str, t_grouped_flags *flags)
{
	...
	printed_chars = 0;
	if (flags &&  (flags->precision_size > (ssize_t)ft_strlen(str) || !(flags->precision)))
		...
		printed_chars = ft_handle_left_padding(flags, flags->precision_size,
				FALSE);
	i = 0;
	while ((str[i] && (!flags || !(flags->precision))) || (flags && flags->precision && str[i]
...
size_t	ft_print_number(long i, t_grouped_flags *flags, t_boolean is_unsigned)
{
	...
	if (decimal_len > flags->precision_size || !(flags->precision))
		printed_chars += ft_handle_left_padding(flags, decimal_len, flags->precision);
	else
		printed_chars += ft_handle_left_padding(flags, flags->precision_size + (i < 0) , flags->precision);
